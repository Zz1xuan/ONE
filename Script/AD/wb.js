  ...
  if (url.includes("/2/cardlist")) {
    ...
    if (url.includes("container_timeline")) {
      ...
      if (obj?.items?.length > 0) {
          ...
          if (item?.category === "feed") {
            ...
            if (item?.item_category === "insert_item") {
              ...
              if (item?.items?.length > 0) {
                let newII = [];
                for (let ii of item.items) {
                  // 118横版广告图片 182热议话题 217错过了热议 247横版视频广告 264微博趋势
        ...
        if (obj?.channelInfo) {
            obj.channelInfo.channels = [];       // 清空频道
            delete obj.channelInfo.moreChannels; // 移除更多版块
            delete obj.channelInfo.channelConfig;// 移除配置
        }
        if (obj?.header) {
            obj.header.data = {};                // 清空 header 数据
        }
        log("clear_finder success");
    }
    // else if (url.includes("finder")) {
    //   if (obj?.channelInfo?.channels?.length > 0) {
    //     let newChannels = [];
    ...
    if (obj?.cards?.length > 0) {
      ...
      obj.cards = newCards;
    }// 13.11.1版本obj下的cards变为items 2023-11-26
    if (obj?.items?.length > 0) {
      let newItems = [];
      for (let item of obj.items) {
        if (!isAd(item?.data)) {
              ...
              for (let ii of item.items) {
                if (ii?.cate_id === "638" && ii?.readtimetype === "card") {
                  ...
                  if (!isAd(ii?.data)) {
                    ...
                    // 3推广卡片 17相关搜索 22广告图 25智搜总结 30推荐博主 42,236智搜问答 89商品推广视频 101都在问 206推广视频
