...
for (let c of a.items)
    ...
    else 0 < c.items.length ? (c.items = Object.values(c.items).filter(a => "feed" == a.category || "card" == a.category), b.push(c)) : b.push(c);
...
for (let e of b) {
    ...
    if (e.mblog) {
        ...
        if ("bottom_mix_activity" == e.itemid) a = !1;
            ...
            if (!b) continue;
            let c = b[0];
            if (-1 < ["guess_like_title", "cats_top_title", "chaohua_home_readpost_samecity_title"].indexOf(c.itemid)) a = !1;
            else if (1 < b.length) {
                let a = [];
...
try {
    let b = a.trend.extra_struct.extBtnInfo.btn_picurl; - 1 < b.indexOf("timeline_icon_ad_delete") && delete a.trend
} catch (a) {}
...
for (let c of a.items) {
    let a = c.itemId;
    if ("profileme_mine" == a) mainConfig.removeHomeVip && (c = removeHomeVip(c)), c.header?.vipIcon && delete c.header.vipIcon, updateFollowOrder(c), b.push(c);
    else if ("100505_-_top8" == a) updateProfileSkin(c, "profileSkin1"), b.push(c);
    else if ("100505_-_newcreator" == a) "grid" == c.type ? (updateProfileSkin(c, "profileSkin2"), b.push(c)) : !mainConfig.removeHomeCreatorTask && b.push(c);
    else if ("100505_-_chaohua" == a || "100505_-_manage" == a || "100505_-_recentlyuser" == a) 0 < c.images?.length && (c.images = c.images.filter(a => "100505_-_chaohua" == a.itemId || "100505_-_recentlyuser" == a.itemId)), b.push(c);
    else continue
}
return a.items = b, a
}
...
if (0 !== c.length) {
    let d = [];
    for (const a of c) {
        let c = a.adType || ""; - 1 == b.indexOf(c) && 6 != a.type && d.push(a)
    }
