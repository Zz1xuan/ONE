    ...
    switch (true) {
        case /api\/sns\/v\d\/note\/widgets/.test($request.url):
            try {
                ...
                let t = ["goods_card_v2", "note_next_step"];
                for (let a of t) {
                    if (e.data?.[a]) {
                        delete e.data[a];
                    }
        ...
        case /api\/sns\/v\d\/note\/redtube/.test($request.url):
            try {
                ...
                for (let d of o.data.items) {
                    if (d.related_goods_num) d.related_goods_num = 0;
                    if (d.has_related_goods) d.has_related_goods = false;
                    if (d.media_save_config) {
                        d.media_save_config = {
                            disable_save: true,
        ...
        case /api\/sns\/v\d\/note\/videofeed/.test($request.url):
            try {
                ...
                for (let l of i.data) {
                    if (l.related_goods_num) l.related_goods_num = 0;
                    if (l.has_related_goods) l.has_related_goods = false;
                    if (l.media_save_config) {
                        l.media_save_config = {
                            disable_save: true,
                            disable_watermark: false,
                            disable_weibo_cover: false
        ...
        case /api\/sns\/v\d\/note\/feed/.test($request.url):
            try {
                ...
                for (let y of c.data) {
                    ...
                    if (y.has_related_goods) y.has_related_goods = false;
                    if (y.note_list) {
        ...
        case /api\/sns\/v\d\/homefeed\/categories\?/.test($request.url):
            try {
                ...
                b.data.categories = b.data.categories.filter(e => !(e.oid == "homefeed.shop" || e.oid == "homefeed.live"));
        ...
        case /api\/sns\/v\d\/system_service\/config\?/.test($request.url):
            try {
                ...
                let C = ["store", "splash", "loading_img", "app_theme", "cmt_words", "highlight_tab"];
                for (let O of C) {
                    if (x.data?.[O]) {
                        delete x.data[O];
                    }
