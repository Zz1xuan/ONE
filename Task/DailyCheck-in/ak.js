/****************************************
@Author: Sliverkiss
@Date: 2023-08-05 12:52:18
@tg频道: https://github.com/Sliverkiss
@Description: 
微信小程序 奥康会员中心 日常签到领积分，积分可以兑换实物
使用教程：
 1.复制Cookie脚本到本地
 2.打开微信小程序->积分明细，若提示获取cookie成功则可以使用签到脚本
 3.关闭获取token脚本

【Loon】 :
*************************
[Script]
cron "0 30 2 * * *" script-path=https://raw.githubusercontent.com/Sliverkiss/helloworld/master/Study/ak.js, timeout=300, tag=奥康会员中心
...
function status() {
    return new Promise((resolve) => {
        const statusRequest = {
            url: `https://wxa-tp.ezrpro.com/myvip/Vip/Bonus/GetMyBonusLogs?pageSize=10&pageIndex=1`,
            headers: env
        };
        $.get(statusRequest, (error, response, data) => {
            var result = JSON.parse(data);
            if (result?.Success == true) {
                message+=`当前积分:${result.Result.BonusTotal}`
            } else {
...
function getCookie() {
    if ($request && $request.method != 'OPTIONS') {
        const env = JSON.stringify($request.headers)
        $.setdata(env, env_name);
