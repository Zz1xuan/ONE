...
脚本名称：微信小程序 大桶订水 签到领现金
脚本作者：@Sliverkiss
更新日期：2023.09.09 08:27:22

2023.09.14 09:18:22 添加提现名字，修复余额满1元后无法提现的问题
...
1.登录后抓包 https://admin.dtds888.com/api/index/user/SignIn请求体的token，填写到dtds_data,多账号用 @ 分割
...
class UserInfo {
    constructor(str) {
        ...
        this.ckStatus = true
        this.headers = {
            'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 14_8 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.40(0x18002831) NetType/WIFI Language/zh_CN',
    ...
    async signin() {
        try {
            const options = {
                ...
                body: `{ "deviceType": "wxapp", "noncestr": "", "timestamp": 0, "sign": "", "token": "${this.token}", "version": "1.00" }`
            };
            //post方法
            let result = await httpRequest(options);
            if (result?.code == 1) {
                //obj.error是0代表完成
                DoubleLog(`✅签到:签到成功！获得${result?.data}元`);
            } else {
    ...
    async cashPost() {
        try {
            const options = {
                ...
                body: `{ "money":"1","name":"a","deviceType": "wxapp", "noncestr": "", "timestamp": 0, "sign": "", "token": "${this.token}", "version": "1.00" }`
            };
            //post方法
            let result = await httpRequest(options);
            console.log(result)
    ...
    async point() {
        ...
        if (result?.code == 1) {
            //obj.error是0代表完成
            DoubleLog(`✅余额:${result?.data?.user?.balance}元`);
            if (result?.data?.user?.balance > 1) {
                //await this.cashPost();
...
async function getNotice() {
    try {
        const urls = ["https://ghporxy.com/https://raw.githubusercontent.com/Sliverkiss/GoodNight/main/notice.json", "https://raw.githubusercontent.com/Sliverkiss/GoodNight/main/tip.json"];
