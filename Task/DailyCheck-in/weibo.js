...
if (typeof $request !== 'undefined') {
    ...
    !(async () => {
        if (!$.isNode() && wbtoken.indexOf("#") === -1) {
            ...
            if ($.isNode()) {
                if (process.env.WB_TOKEN?.includes('#')) {
                    wbtoken = process.env.WB_TOKEN.split('#');
                } else if (process.env.WB_TOKEN?.includes('\n')) {
                    wbtoken = process.env.WB_TOKEN.split('\n');
                } else {
                    wbtoken = [process.env.WB_TOKEN];
                }

                if (process.env.WB_COOKIE?.includes('#')) {
                    cookies = process.env.WB_COOKIE.split('#');
                } else if (process.env.WB_COOKIE?.includes('\n')) {
                    cookies = process.env.WB_COOKIE.split('\n');
                } else {
                    cookies = [process.env.WB_COOKIE];
                }
            } else if (!$.isNode() && wbtoken.includes("#")) {
                wbtoken = wbtoken.split("#");
                cookies = cookies.split("#");
            }

            wbtoken.forEach(item => { if (item) tokenArr.push(item); });
            cookies.forEach(item => { if (item) cookieArr.push(item); });
        }
        ...
        const timestamp = Date.now() + (8 + timeZone) * 60 * 60 * 1000;
        const bjTime = new Date(timestamp).toLocaleString('zh', { hour12: false, timeZoneName: 'long' });
        ...
        for (let i = 0; i < tokenArr.length; i++) {
            if (tokenArr[i]) {
                ...
                $.index = i + 1;
                console.log(`\n开始【微博签到${$.index}】`);
                if (!token.includes("from")) token = "&from=10B3193010" + token;
                await getsign();
                await paysign();
                await getcash();
                await payinfo();
...
function GetCookie() {
    if ($request && $request.method !== 'OPTIONS') {
        if ($request.url.includes("gsid=")) {
            const signurlVal = $request.url;
            const matchedToken = signurlVal.match(/from=\w+.*?uid=\d+.*?gsid=[_a-zA-Z0-9-]+.*?s=\w+/);
            if (!matchedToken) return;
            const token = matchedToken[0];
            const uid = token.match(/uid=(\d+)/)?.[1];
            if (!uid) return;

            const oldTokens = $.getdata('sy_token_wb') || "";
            const tokenList = oldTokens.split("#").filter(Boolean);
            const hasExist = tokenList.some(t => t.includes(`uid=${uid}`));
            if (!hasExist) {
                tokenList.push(token);
                $.setdata(tokenList.join("#"), 'sy_token_wb');
                $.msg($.name, `✅ 成功获取微博 Token`, `UID: ${uid}\n共 ${tokenList.length} 个账号`);
            } else {
                $.log(`🔁 UID ${uid} 的 Token 已存在，跳过`);
            }
        }

        const cookieHeader = $request.headers?.Cookie || "";
        if (cookieHeader.includes("SUB=")) {
            const cookieVal = cookieHeader.match(/SUB=[\w-]+/)?.[0];
            if (!cookieVal) return;

            const oldCookies = $.getdata('wb_cookie') || "";
            const cookieList = oldCookies.split("#").filter(Boolean);
            const hasExist = cookieList.includes(cookieVal);
            if (!hasExist) {
                cookieList.push(cookieVal);
                $.setdata(cookieList.join("#"), 'wb_cookie');
                $.msg($.name, `✅ 成功获取微博 Cookie`, `共 ${cookieList.length} 个账号`);
            } else {
                $.log("🔁 Cookie 已存在，跳过");
            }
        }
