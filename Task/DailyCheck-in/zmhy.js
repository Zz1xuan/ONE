...
脚本名称：微信小程序 中免会员小程序 签到三天抽盲盒，1积分=1元
脚本作者：@Sliverkiss
更新日期：2023.09.10 15:02:22
...
青龙:
1.登录后抓包 cdfmbrapi.cdfg.com.cn域名下的x-access-token，填写到zmhy_data,多账号用 @ 分割
2.可选推送：将bark的key填写到bark_key，不填默认使用青龙自带的推送

loon:
1.将获取cookie脚本拉取到本地
2.打开小程序->我的->积分明细,若提示获取token成功则可以使用该脚本
3.关闭获取ck脚本，避免产生不必要的mitm

[Script]
cron "3 0 * * *" script-path=https://raw.githubusercontent.com/Sliverkiss/GoodNight/master/Script/zmhy.js, timeout=300, tag=中免会员小程序
http-request ^https:\/\/cdfmbrapi.cdfg.com.cn\/api\/points\/paymentList script-path=https://raw.githubusercontent.com/Sliverkiss/GoodNight/master/Script/zmhy.js, timeout=10, tag=中免会员小程序获取token
[MITM]
...
// env.js 全局
const $ = new Env("中免会员小程序");
const ckName = "zmhy_data";
//-------------------- 一般不动变量区域 -------------------------------------
...
class UserInfo {
    constructor(str) {
        ...
        this.ckStatus = true
        this.headers = {
            'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 14_8 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.41(0x1800292d) NetType/WIFI Language/zh_CN',
    ...
    async signin() {
        try {
            const options = {
                //签到任务调用签到接口
                url: `https://cdfmbrapi.cdfg.com.cn/api/user/sign`,
                //请求头, 所有接口通用
                headers: this.headers,
                body: `repairSignDate=&randomizedIdMap=&lng=&lat=&repairSignType=`
            };
            //post方法
    ...
    async point() {
        let signinRequest = {
            //签到任务调用签到接口
            url: `https://cdfmbrapi.cdfg.com.cn/api/user/info`,
            //请求头, 所有接口通用
            headers: this.headers
        };
        //post方法
        ...
        if (result?.code == 1) {
            //obj.error是0代表完成
            DoubleLog(`✅余额:${result?.data?.points?.balance}积分`);
        } else {
...
async function getCookie() {
    if ($request && $request.method != 'OPTIONS') {
        const tokenValue = $request.headers['x-access-token'] || $request.headers['X-Access-Token']|| $request.headers['X-ACCESSS-TOKEN'];
        if (tokenValue) {
