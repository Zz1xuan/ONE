...
YouTube Music lyrics enhancer (Final Version)
目标：优化已有歌词显示（净化、简体化），并尝试替换“点进去后才显示没有歌词”的面板。对于“真无歌词”的歌曲，脚本将明确提示无法处理。

[mitm]
hostname = youtubei.googleapis.com, lrclib.net, music.163.com, translate.googleapis.com
...
function handleGetWatch(url, raw, text) {
  ...
  const hasLyricsPanelMarker = text.includes('music_watch_lyrics_panel');
  const hasTimedLyricsController = text.includes('timed_lyrics_controller');
  const hasNoLyricsText = text.includes('没有歌词');
  const title = guessTitle(text);
  const artist = guessArtist(text);
  const meta = { videoId, title, artist, ts: Date.now() };
  $.write(JSON.stringify(meta), META_KEY);
  ...
  $.log(`hasLyricsPanelMarker=${hasLyricsPanelMarker}`);
  $.log(`hasTimedLyricsController=${hasTimedLyricsController}`);
  $.log(`hasNoLyricsText=${hasNoLyricsText}`);

  if (hasNoLyricsText && !hasTimedLyricsController && !hasLyricsPanelMarker) {
    $.log('WATCH RESULT: True no-lyrics song, cannot be enhanced.');
  } else if (hasNoLyricsText && hasLyricsPanelMarker) {
    $.log('WATCH RESULT: Likely no-lyrics panel, will try to replace content.');
  } else {
    $.log('WATCH RESULT: Lyrics content expected, will try to enhance.');
  }
  return $.done({ bodyBytes: sliceBytes(raw) });
}

async function handleGetPanel(url, raw, text) {
  $.log('===== GET_PANEL ENHANCE =====');
  $.log(`url=${url}`);
  $.log(`rawBytes=${raw.byteLength}`);

  const meta = readMeta();
  $.log(`meta.title=${meta.title || 'unknown'} meta.artist=${meta.artist || 'unknown'}`);

  const noLyricsIdx = text.indexOf('没有歌词');
  const hasTimedLyrics = text.includes('timed_lyrics_controller');
  const hasLyricFind = text.includes('LyricFind') || text.includes('来源：');
  $.log(`noLyricsIdx=${noLyricsIdx} hasTimedLyrics=${hasTimedLyrics} hasLyricFind=${hasLyricFind}`);

  let originalLines = [];
  let targetLines = [];
  let sourceName = 'Original';

  // Mode 1: Empty lyrics panel (noLyricsIdx >= 0)
  if (noLyricsIdx >= 0) {
    $.log('MODE=no_lyrics_panel_replace');
    originalLines = collectNearChinese(text, noLyricsIdx, 16);
    targetLines = await fetchExternalLyrics(meta, originalLines.slice(0, 4)).catch(e => { $.log(`External fetch error: ${e.message}`); return []; });
    if (!targetLines.length) {
      targetLines = await translateLines(originalLines.slice(0, 8)).catch(e => { $.log(`Translate error: ${e.message}`); return []; });
      sourceName = 'Google Translate';
    }
    if (!targetLines.length) {
      $.log('No external/translated lyrics, keep original no-lyrics panel.');
      return $.done({ bodyBytes: sliceBytes(raw) });
    }
  } else { // Mode 2: Normal lyrics panel (noLyricsIdx < 0)
    $.log('MODE=normal_lyrics_enhance');
    const probe = buildProbe(text);
    originalLines = probe.cleanPairs.map(p => p.cleaned);
    targetLines = await fetchExternalLyrics(meta, originalLines.slice(0, 4)).catch(e => { $.log(`External fetch error: ${e.message}`); return []; });
    if (!targetLines.length && originalLines.length > 0) {
      // Fallback to translate if original lines are not Chinese only
      const nonChineseOriginal = originalLines.filter(l => !/[\u4E00-\u9FFF]/.test(l));
      if (nonChineseOriginal.length > 0) {
        targetLines = await translateLines(nonChineseOriginal.slice(0, MAX_TRANSLATE_LINES)).catch(e => { $.log(`Translate error: ${e.message}`); return []; });
        sourceName = 'Google Translate';
      }
    }
    if (!targetLines.length) {
      $.log('No external/translated lyrics, applying only simplification to original.');
      // Just simplify existing original lines if no external source
      let replaced = 0;
      for (const pair of probe.cleanPairs) {
        const from = pair.cleaned;
        const simp = normalizeLine(toSimplified(normalizeLine(from)));
        if (!simp || simp === from) continue;
        ...
        if (hit.replaced) { replaced++; $.log(`SIMPLIFIED ${replaced}: ${from} -> ${safeDecode(toBytes)} @ ${hit.index}`); }
      }
      $.log(`totalSimplified=${replaced}`);
      return $.done({ bodyBytes: sliceBytes(raw) });
    }
  }

  // Apply replacement for both modes
  let replacedCount = 0;
  $.log(`Applying ${sourceName} lyrics. Original count: ${originalLines.length}, Target count: ${targetLines.length}`);
  for (let i = 0; i < Math.min(originalLines.length, targetLines.length); i++) {
    const from = originalLines[i];
    const to = targetLines[i];
    const toDisplay = normalizeLine(toSimplified(to)); // Ensure target is simplified too
    if (!from || !toDisplay || from === toDisplay) continue;
...
function readMeta() {
  try { return JSON.parse($.read(META_KEY) || '{}'); } catch { return {}; }
}

function guessTitle(text) {
  const m = text.match(/title.{0,120}(?:runs|text)[^\x00]{0,220}/i);
  if (m) return cleanupMeta(m[0]);
  const cands = collectReadableLines(text, 10);
  return cands.find(s => /[A-Za-z\u4E00-\u9FFF]/.test(s) && s.length > 1) || '';
}

function guessArtist(text) {
  const m = text.match(/shortBylineText.{0,120}(?:runs|text)[^\x00]{0,220}/i);
  if (m) return cleanupMeta(m[0]);
  const cands = collectReadableLines(text, 10);
  return cands.find(s => /[A-Za-z\u4E00-\u9FFF]/.test(s) && s.length > 1) || '';
}
...
function cleanupMeta(s) {
  let x = String(s || '');
  x = x.replace(/title|標題|标题|shortBylineText|runs|text|:{|\d+/g, ' ');
  x = x.replace(/[\u0000-\u001F]/g, ' ').replace(/\s{2,}/g, ' ').trim();
  return x.slice(0, 120);
}

async function fetchExternalLyrics(meta, queryLines) {
  let sourceLines = [];
  let sourceName = '';

  // Try LRCLIB first if title/artist available
  if (meta.title && meta.artist) {
    sourceLines = await fetchLrclib(meta).catch(e => { $.log(`LRCLIB fetch error: ${e.message}`); return []; });
    if (sourceLines.length) sourceName = 'LRCLIB';
  }

  // Fallback to Netease if LRCLIB fails or no result
  if (!sourceLines.length && meta.title) {
    sourceLines = await fetchNetease(meta, queryLines).catch(e => { $.log(`Netease fetch error: ${e.message}`); return []; });
    if (sourceLines.length) sourceName = 'NETEASE';
  }

  if (sourceLines.length) {
    $.log(`EXTERNAL SOURCE: ${sourceName}, lines=${sourceLines.length}`);
    return sourceLines.map(s => normalizeLine(toSimplified(s)));
  }
  return [];
}

async function fetchLrclib(meta) {
  const q = `https://lrclib.net/api/search?track_name=${encodeURIComponent(meta.title||'')}&artist_name=${encodeURIComponent(meta.artist||'')}`;
  const resp = await $task.fetch({ url:q, method:'GET' });
  if (!resp || resp.statusCode !== 200) throw new Error(`LRCLIB HTTP ${resp?.statusCode}`);
  const arr = JSON.parse(resp.body);
  if (!Array.isArray(arr) || !arr.length) return [];
  const item = arr[0] || {};
  const lyric = item.plainLyrics || item.syncedLyrics || '';
  return parsePlainLyrics(lyric);
}

async function fetchNetease(meta, queryLines) {
  // Netease search is very sensitive, try with title+artist, then title only, then queryLines
  let searchQueries = [];
  if (meta.title && meta.artist) searchQueries.push(`${meta.title} ${meta.artist}`);
  if (meta.title) searchQueries.push(meta.title);
  if (queryLines && queryLines.length > 0) searchQueries.push(queryLines[0]); // Use a line as query

  for (const qText of searchQueries) {
    if (!qText) continue;
    const q = `https://music.163.com/api/search/pc?s=${encodeURIComponent(qText)}&offset=0&limit=5&type=1`;
    const resp = await $task.fetch({ url:q, method:'GET', headers:{'User-Agent':'Mozilla/5.0','Referer':'https://music.163.com'} });
    if (!resp || resp.statusCode !== 200) { $.log(`Netease search HTTP ${resp?.statusCode} for ${qText}`); continue; }
    const data = JSON.parse(resp.body);
    const song = data?.result?.songs?.[0];
    if (!song?.id) { $.log(`Netease no song ID for ${qText}`); continue; }

    const lr = await $task.fetch({ url:`https://music.163.com/api/song/lyric?id=${song.id}&lv=0&yv=0&tv=0`, method:'GET', headers:{'User-Agent':'Mozilla/5.0','Referer':'https://music.163.com'} });
    if (!lr || lr.statusCode !== 200) { $.log(`Netease lyric fetch HTTP ${resp?.statusCode} for ${song.id}`); continue; }
    const lyr = JSON.parse(lr.body);
    const lyrics = parseLrc(lyr?.lrc?.lyric || lyr?.tlyric?.lyric || '');
    if (lyrics.length) { $.log(`Netease found lyrics for ${qText}`); return lyrics; }
  }
  return [];
}

function parsePlainLyrics(s) { return String(s||'').split(/\n+/).map(x => normalizeLine(toSimplified(x))).filter(Boolean).slice(0, MAX_LRCLIB_LINES); }
function parseLrc(s) { return String(s||'').split(/\n+/).map(x => x.replace(/\[[^\]]+\]/g,'')).map(x => normalizeLine(toSimplified(x))).filter(Boolean).slice(0, MAX_NETEASE_LINES); }
async function translateLines(lines) {
  ...
  if (!resp || resp.statusCode !== 200) throw new Error(`Translate HTTP ${resp?.statusCode}`);
  const data = JSON.parse(resp.body);
  return ((data[0] || []).map(x => x[0] || '').join('')).split('\n').map(x => normalizeLine(toSimplified(x))).filter(Boolean).slice(0, MAX_TRANSLATE_LINES);
}
...
function buildProbe(text) {
  ...
  let bestAnchor = null, bestScore = -1, bestPairs = [];
  for (const [name,before,after] of anchors) {
    const idx = text.indexOf(name); if (idx < 0) continue;
    const pairs = collectPairs(text.slice(Math.max(0, idx-before), Math.min(text.length, idx+after)), 80);
    const score = scorePairs(pairs);
    if (score > bestScore) { bestScore = score; bestAnchor = { name, index: idx, score, count: pairs.length }; bestPairs = pairs; }
  }
  const cleanPairs = mergePairs(bestPairs).filter(p => isLikelyLyric(p.cleaned)).slice(0, 60);
...
function collectPairs(block, limit) {
  ...
  for (const raw of block.split(/[\n\r\x00]/).map(s=>s.trim()).filter(Boolean)) {
    ...
    seen.add(cleaned); out.push({ raw: compact(raw), cleaned }); if (out.length >= limit) break;
  }
  return out;
}
function mergePairs(arr) { const out=[], seen=new Set(); for (const p of arr) { if (!p.cleaned || seen.has(p.cleaned)) continue; seen.add(p.cleaned); out.push(p); } return out; }
function scorePairs(pairs) { let score=0; for (const p of pairs) { if (isSystemNoise(p.cleaned)) { score -= 8; continue; } if (isLikelyLyric(p.cleaned)) score += 5; if (/[\u4E00-\u9FFF]/.test(p.cleaned)) score += 4; if (/^[0-9]/.test(p.cleaned)) score -= 2; if (/�/.test(p.cleaned)) score -= 3; } return score; }
function cleanLyricLine(s) { let x=String(s||''); x=x.replace(/[\u0000-\u001F]/g,' ').replace(/[\x7F-\x9F]/g,' ').replace(/^["'`*%#,+.;:!?，。！？、<>=\-_=|\\/\s]+/g,'').replace(/["'`*%#,+.;:!?，。！？、<>=\-_=|\\/\s]+$/g,'').replace(/^[0-9](?=[A-Za-z\u4E00-\u9FFF])/g,'').replace(/^[^\p{L}\p{N}\u4E00-\u9FFF\u3040-\u30FF\uAC00-\uD7AF]+/gu,'').replace(/\s{2,}/g,' ').trim().replace(/�+/g,'').trim().replace(/[A-Za-zΑ-Ωα-ωƛǎǐǒǔǚàèìòùáéíóúüńňňḿǹȁȅȉȍȕˇˋˊˉ˙]+$/g,'').trim(); return x; }
function isSystemNoise(s) { return /has_unlimited_entitlement|logged_in|client\.version|client\.name|lyrics_collection|IOS_MUSIC|entity|collection|button|controller|panel/i.test(s); }
function isLikelyLyric(s) { if (!s) return false; if (s.length < 4 || s.length > 80) return false; if (isSystemNoise(s)) return false; if (/歌詞?b|lyrics?b|Ӎ|˳ɔ|q&Ғ|IOS_MUSIC|logged_in|client\.version|client\.name/i.test(s)) return false; if (/^[A-Za-z0-9_\-|]{10,}$/.test(s)) return false; if (/^[\-–—_=+*/|\\:;,.]+$/.test(s)) return false; if (!/[A-Za-z\u4E00-\u9FFF\u3040-\u30FF\uAC00-\uD7AF]/.test(s)) return false; const zh=(s.match(/[\u4E00-\u9FFF]/g)||[]).length; const bad=(s.match(/[�\[\]{}<>]/g)||[]).length; if (bad >= 2) return false; if (zh >= 4) return true; if (/\s/.test(s) && /[A-Za-z]/.test(s)) return true; return false; }
function normalizeLine(s) { let x=String(s||'').replace(/\s{2,}/g,' ').trim(); x=x.replace(/怎幺/g,'怎么').replace(/前一秒鍾/g,'前一秒钟').replace(/一秒鍾/g,'一秒钟').replace(/裏/g,'里').replace(/囉嗦/g,'啰嗦').replace(/放鬆/g,'放松').replace(/夢/g,'梦').replace(/隱秘/g,'隐秘'); return x; }
function toSimplified(str) { let out=''; for (const ch of String(str)) out += (T2S_MAP[ch] || ch); return out; }
function compact(s) { return String(s||'').replace(/[\u0000-\u001F]/g,' ').replace(/\s{2,}/g,' ').trim().slice(0,120); }
function safeDecode(u8) { try { return new TextDecoder('utf-8',{fatal:false}).decode(u8); } catch(e) { let s=''; for(let i=0;i<u8.length;i++) s+=String.fromCharCode(u8[i]); try { return decodeURIComponent(escape(s)); } catch(_) { return s; } } }
function utf8Bytes(str) { return new TextEncoder().encode(str); }
function fitToByteLength(str, byteLen) { let out=str; while (utf8Bytes(out).length > byteLen && out.length > 0) out=out.slice(0,-1); while (utf8Bytes(out).length < byteLen) out+=' '; let bytes=utf8Bytes(out); if (bytes.length > byteLen) bytes=bytes.slice(0,byteLen); if (bytes.length < byteLen) { const pad=new Uint8Array(byteLen); pad.set(bytes,0); for (let i=bytes.length;i<byteLen;i++) pad[i]=32; bytes=pad; } return bytes; }
function replaceFirstBytes(haystack, needle, repl) { const idx=indexOfBytes(haystack,needle); if (idx < 0 || needle.length !== repl.length) return {replaced:false,index:-1}; haystack.set(repl, idx); return {replaced:true,index:idx}; }
function indexOfBytes(haystack, needle) { outer: for (let i=0;i<=haystack.length-needle.length;i++) { for (let j=0;j<needle.length;j++) if (haystack[i+j]!==needle[j]) continue outer; return i; } return -1; }
function sliceBytes(u8) { return u8.buffer.slice(u8.byteOffset, u8.byteOffset + u8.byteLength); }
function Env(name) { return { log: msg => console.log(`[${name}] ${msg}`), done: val => $done(val), read: key => $prefs.valueForKey(key), write: (val,key) => $prefs.setValueForKey(val,key) }; }
