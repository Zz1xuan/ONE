...
const $ = new Env('YTM Lyrics Force Inject');
...
function handleGetWatch(url, raw, text) {
  ...
  const nearLyricsPanel = /music_watch_lyrics_panel.{0,220}(igsR|igsS|lyrics-button|quote_24pt|歌词)/s.test(text);
  const nearLyricsButton = /music-lyrics-button.{0,180}(歌词|quote_24pt|music_watch_lyrics_panel)/s.test(text);
  const strongLyricsEntry = nearLyricsPanel && nearLyricsButton;
  ...
  $.log(`nearLyricsPanel=${nearLyricsPanel}`);
  $.log(`nearLyricsButton=${nearLyricsButton}`);
  $.log(`strongLyricsEntry=${strongLyricsEntry}`);

  if (strongLyricsEntry) {
    $.log('WATCH RESULT: strong lyrics entry detected by strings, but continue force injection experiment');
  }
