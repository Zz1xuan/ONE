...
用途：拦截 YouTube Music 歌词面板（get_panel），优先将可命中的原歌词统一为简体；对非中文/混合行再做翻译替换。
...
说明：
1. 这是一版 v9 覆盖率优先脚本，先尽量把命中的中文繁体歌词统一成简体，再对非中文/混合行做翻译替换。
2. 采用“等字节长度替换”，不改 protobuf 结构长度，只做局部文本替换。
3. 当前目标是让可命中的歌词文本尽量统一成简体，提高可见效果。
4. 若命中失败，脚本会原样放行。

*/

const $ = new Env('YTM Lyrics Prototype v9');
const MAX_TRANSLATE_LINES = 14;
...
(async () => {
  try {
    ...
    $.log('=== YTM lyrics panel detected (v8) ===');
    ...
    for (const pair of probe.linePairs) {
      const from = pair.cleaned;
      const simp = normalizeSimplifiedOnly(from);
      ...
      if (hit.replaced) {
        simplifiedCount++;
        $.log(`SIMPLIFIED ${simplifiedCount}: ${from} -> ${safeDecode(toBytes)} @ ${hit.index}`);
      }
    }
    $.log(`Total simplified: ${simplifiedCount}`);

    const targets = pickTargets(probe.linePairs, MAX_TRANSLATE_LINES);
    if (!targets.length) {
      $.log('No translation targets found, return simplified result');
      return $.done({ bodyBytes: sliceBytes(raw) });
    }

    $.log(`Selected translation lines: ${targets.length}`);
    targets.forEach((t, i) => $.log(`${i + 1}. ${t.cleaned}`));

    const translated = await translateLines(targets.map(t => t.cleaned));
    if (!translated.length) {
      $.log('Translation returned empty result, return simplified result');
      return $.done({ bodyBytes: sliceBytes(raw) });
    }
    ...
    $.log(`Total translated: ${replacedCount}`);
    return $.done({ bodyBytes: sliceBytes(raw) });
  } catch (e) {
...
function buildProbe(text) {
  ...
  for (const [name, before, after] of anchors) {
    ...
    if (score > bestScore) bestAnchor = { name, index: idx, score, count: cand.length }, bestScore = score;
...
function normalizeSimplifiedOnly(s) {
  let out = String(s || '').trim();
  if (!out) return '';
  out = toSimplified(out).replace(/\s{2,}/g, ' ').trim();
  return out;
}
...
function normalizeTranslated(s, original) {
  let out = String(s || '').trim();
  if (!out) return '';
  if (out === original) return normalizeSimplifiedOnly(out);
