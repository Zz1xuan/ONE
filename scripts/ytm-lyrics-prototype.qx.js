...
用途：YouTube Music 歌词增强 v11。缓存当前视频元信息，优先尝试 LRCLIB / 网易云歌词，失败再回退机翻。

------------ Quantumult X 配置 ------------

[mitm]
hostname = youtubei.googleapis.com, translate.googleapis.com, lrclib.net, music.163.com, www.youtube.com
...
说明：
1. get_watch 用来缓存 videoId/title/artist；get_panel 用来替换歌词。
2. 优先级：LRCLIB / 网易云 → Google 翻译。
3. 仍采用等字节长度替换，不改 protobuf 结构长度。

*/

const $ = new Env('YTM Lyrics Prototype v11');
const META_KEY = 'ytm_last_meta_v11';
const MAX_LINES = 16;
...
(async () => {
  try {
    ...
    if (/youtubei\.googleapis\.com\/youtubei\/v1\/get_watch\?id=/.test(url)) {
      return handleGetWatch(url, new Uint8Array($response.bodyBytes));
    }
    if (/youtubei\.googleapis\.com\/youtubei\/v1\/get_panel/.test(url)) {
      return handleGetPanel(new Uint8Array($response.bodyBytes));
    }
...
function handleGetWatch(url, raw) {
  ...
  $.log(`CACHE META: ${title || 'unknown'} / ${artist || 'unknown'} / ${videoId || 'unknown'}`);
  return $.done({ bodyBytes: sliceBytes(raw) });
}

async function handleGetPanel(raw) {
  const text = safeDecode(raw);
  ...
  const meta = readMeta();
  $.log(`META: ${meta.title || 'unknown'} / ${meta.artist || 'unknown'} / ${meta.videoId || 'unknown'}`);

  let sourceLines = [];
  let sourceName = '';

  if (meta.title) {
    const lrc = await fetchLrclib(meta).catch(() => null);
    if (lrc && lrc.length) {
      sourceLines = lrc;
      sourceName = 'LRCLIB';
    }
  ...
  if (!sourceLines.length && meta.title) {
    const n163 = await fetchNetease(meta).catch(() => null);
    if (n163 && n163.length) {
      sourceLines = n163;
      sourceName = 'NETEASE';
    }
  ...
  if (!sourceLines.length) {
    const targets = pickTranslateTargets(probe.linePairs, MAX_LINES);
    if (!targets.length) return $.done({ bodyBytes: sliceBytes(raw) });
    const translated = await translateLines(targets.map(t => t.cleaned)).catch(() => []);
    if (!translated.length) return $.done({ bodyBytes: sliceBytes(raw) });
    sourceLines = translated.map(s => normalizeText(s)).filter(Boolean);
    sourceName = 'GOOGLE';
    applyLineMap(raw, targets.map(t => t.cleaned), sourceLines);
    $.log(`SOURCE: ${sourceName}, replaced=${sourceLines.length}`);
    return $.done({ bodyBytes: sliceBytes(raw) });
  }

  const originalTargets = probe.linePairs.map(x => x.cleaned).filter(Boolean).slice(0, Math.min(sourceLines.length, MAX_LINES));
  applyLineMap(raw, originalTargets, sourceLines.slice(0, originalTargets.length));
  $.log(`SOURCE: ${sourceName}, replaced=${Math.min(sourceLines.length, originalTargets.length)}`);
  return $.done({ bodyBytes: sliceBytes(raw) });
}
...
function guessTitle(text) {
  const m = text.match(/music_detail_header[^\x00]{0,300}?(?:title|標題|标题)[^\x00]{0,80}/i);
  if (m) return cleanupMeta(m[0]);
  const cands = collectTextCandidates(text, 80);
  return cands.find(s => /[A-Za-z\u4E00-\u9FFF\u3040-\u30FF\uAC00-\uD7AF]/.test(s) && s.length > 1) || '';
}

function guessArtist(text) {
  const m = text.match(/by [A-Za-z0-9 &,'._\-]{2,80}/i);
...
function collectTextCandidates(text, limit) {
  return text.split(/[\n\r\x00]/).map(s=>s.trim()).filter(Boolean).filter(s=>s.length<100).slice(0,limit);
}
...
function buildProbe(text) {
  ...
  const isLyricsPanel = hasAny(text, ['music_watch_lyrics_panel','timed_lyrics_controller','来源：','LyricFind','lyric_line.eml-fe']);
  let bestAnchor = null, bestScore = -1;
  const seen = new Set(), linePairs = [];
  for (const [name,before,after] of anchors) {
    const idx = text.indexOf(name); if (idx < 0) continue;
    const block = text.slice(Math.max(0, idx-before), Math.min(text.length, idx+after));
    const cand = collectPairs(block); const score = cand.length;
    if (score > bestScore) { bestScore = score; bestAnchor = { name, index: idx, score, count: cand.length }; }
    for (const pair of cand) { if (seen.has(pair.cleaned)) continue; seen.add(pair.cleaned); linePairs.push(pair); }
  }
  if (linePairs.length < 80) {
    const extra = collectPairs(text.slice(0, Math.min(text.length, 140000)));
    for (const pair of extra) { if (seen.has(pair.cleaned)) continue; seen.add(pair.cleaned); linePairs.push(pair); }
  }
...
function collectPairs(block) {
  ...
  for (const raw of parts) {
    ...
    seen.add(cleaned); out.push({ raw: compactRaw(raw), cleaned }); if (out.length >= 200) break;
  }
  return out;
}

function pickTranslateTargets(pairs, limit) {
  const out = [];
...
async function fetchNetease(meta) {
  const q = `https://music.163.com/api/search/pc?s=${encodeURIComponent((meta.title||'') + ' ' + (meta.artist||''))}&offset=0&limit=5&type=1`;
  ...
  if (!lr || lr.statusCode !== 200) return [];
  const lyr = JSON.parse(lr.body);
  return parseLrc(lyr?.lrc?.lyric || '');
}

function parsePlainLyrics(s) {
  return String(s||'').split(/\n+/).map(normalizeText).filter(Boolean).slice(0, MAX_LINES);
}
function parseLrc(s) {
  return String(s||'').split(/\n+/).map(x=>x.replace(/\[[^\]]+\]/g,'')).map(normalizeText).filter(Boolean).slice(0, MAX_LINES);
}
...
function normalizeText(s) {
  ...
  if (!out) return '';
  out = toSimplified(out).replace(/\s{2,}/g,' ').replace(/[\u0000-\u001F]/g,'').trim();
  return out;
}
function toSimplified(str) { let out=''; for (const ch of String(str)) out += (T2S_MAP[ch] || ch); return out; }
function cleanLyricLine(s) { return s.replace(/[\u0001-\u001F]/g,' ').replace(/[\x7F-\x9F]/g,' ').replace(/^["'`*%#,+.;:!?，。！？、\-_=|\\/\s]+/g,'').replace(/["'`*%#,+.;:!?，。！？、\-_=|\\/\s]+$/g,'').replace(/^[0-9](?=[A-Za-z])/g,'').replace(/^[^\p{L}\p{N}\u4E00-\u9FFF\u3040-\u30FF\uAC00-\uD7AF]+/gu,'').replace(/[^\p{L}\p{N}\u4E00-\u9FFF\u3040-\u30FF\uAC00-\uD7AF：:，,。.!?！？'"（）()\-\s]+$/gu,'').replace(/\s{2,}/g,' ').trim(); }
function compactRaw(s) { return s.replace(/[\u0000-\u001F]/g,' ').replace(/\s{2,}/g,' ').trim().slice(0,120); }
function isLyricLine(s) { if (!s) return false; if (s.length < 2 || s.length > 100) return false; if (/music_watch_lyrics_panel|timed_lyrics|LyricFind|controller|button|panel|youtube|eml-fe|eml-js-fe|createTimedLyricsController|destroyTimedLyricsController/i.test(s)) return false; if (/^[A-Za-z0-9_\-|]{10,}$/.test(s)) return false; if (/^[\-–—_=+*/|\\:;,.]+$/.test(s)) return false; if (/^[^A-Za-z\u4E00-\u9FFF\u3040-\u30FF\uAC00-\uD7AF0-9]+$/.test(s)) return false; return /[A-Za-z\u4E00-\u9FFF\u3040-\u30FF\uAC00-\uD7AF]/.test(s); }
...
function hasAny(text, arr) { return arr.some(s => text.includes(s)); }
