...
原型作者：Minis
用途：YouTube Music 歌词净化 + 简体优先 + 双语增强版。
目标：保留 get_panel 里现有净化/简转成果；对非中文歌词尝试机翻，并优先做双语增强。
...
说明：
1. 中文歌词继续走净化 + 简体优先。
2. 非中文歌词尝试调用 Google Translate。
3. 双语增强当前采用“安全替换优先”策略：能安全替就替，不能安全拼接就回退单行翻译。

*/

const $ = new Env('YTM Lyrics Bilingual v13');
const COMMON_T2S_PHRASES = [
  ['怎幺','怎么'],['為什麼','为什么'],['爲什麼','为什么'],['這樣','这样'],['那樣','那样'],
  ['裏','里'],['裡','里'],['裡面','里面'],['這裡','这里'],['那裡','那里'],
  ['夢','梦'],['愛','爱'],['會','会'],['點','点'],['開','开'],['關','关'],['讓','让'],
  ['對','对'],['於','于'],['與','与'],['還','还'],['說','说'],['聽','听'],['見','见'],
  ['後來','后来'],['時候','时候'],['時光','时光'],['時分','时分'],['時刻','时刻'],
  ['前一秒鍾','前一秒钟'],['一秒鍾','一秒钟'],['鍾','钟'],['鐘','钟'],
  ['囉嗦','啰嗦'],['囉','啰'],['嗎','吗'],['誰','谁'],['個','个'],['們','们'],['妳','你'],
  ['放鬆','放松'],['隱秘','隐秘'],['歡迎','欢迎'],['無盡','无尽'],['滿天','满天'],['燈火','灯火'],
  ['微涼','微凉'],['溫柔','温柔'],['熱烈','热烈'],['遙遠','遥远'],['遺憾','遗憾'],['擁抱','拥抱'],
  ['寂寞','寂寞'],['憂傷','忧伤'],['淚','泪'],['淚水','泪水'],['靈魂','灵魂'],['孤獨','孤独'],
  ['陪著','陪着'],['愛著','爱着'],['想著','想着'],['看著','看着'],['聽著','听着'],['唱著','唱着'],
  ['活著','活着'],['笑著','笑着'],['哭著','哭着'],['寫著','写着'],['等著','等着'],['念著','念着'],['著我','着我'],['著你','着你']
];
...
(async () => {
  try {
    ...
    $.log(`anchor=${probe.bestAnchor ? probe.bestAnchor.name : 'none'}`);
    $.log(`cleanPairs=${probe.cleanPairs.length}`);

    probe.cleanPairs.slice(0, 16).forEach((p, i) => {
    ...
    for (const pair of probe.cleanPairs) {
      ...
      const normalized = normalizeLine(from);
      const simp = normalizeLine(toSimplified(normalized));
      if (simp && simp !== from) {
      ...
      if (!shouldTranslate(normalized)) continue;
      const tr = await translateLine(normalized, 'zh-CN');
      if (!tr || tr === normalized) continue;
      translatedMap.set(from, normalizeTranslation(tr));
    }

    for (const pair of probe.cleanPairs) {
      const from = pair.cleaned;
      const tr = translatedMap.get(from);
      if (!tr) continue;
      const translatedLine = buildBilingualLine(from, tr);
      const fromBytes = utf8Bytes(from);
      const toBytes = fitToByteLength(translatedLine, fromBytes.length);
      const hit = replaceFirstBytes(raw, fromBytes, toBytes);
      if (hit.replaced) {
        translated++;
        $.log(`TRANSLATED ${translated}: ${from} -> ${safeDecode(toBytes)} @ ${hit.index}`);
      }
    }

    $.log(`totalSimplified=${replaced}`);
    $.log(`totalTranslated=${translated}`);
...
function buildProbe(text) {
  ...
  for (const [name,before,after] of anchors) {
    ...
    const pairs = collectPairs(block, 80);
    const score = scorePairs(pairs);
    if (score > bestScore) {
      bestScore = score;
      bestAnchor = { name, index: idx, score, count: pairs.length };
      bestPairs = pairs;
    }
  }
  if (bestPairs.length < 24) {
...
function mergePairs(arr) {
  ...
  const seen = new Set();
  for (const p of arr) {
    if (!p.cleaned || seen.has(p.cleaned)) continue;
    seen.add(p.cleaned);
    out.push(p);
  }
  return out;
...
function scorePairs(pairs) {
  ...
  for (const p of pairs) {
    if (isSystemNoise(p.cleaned)) { score -= 8; continue; }
    if (isLikelyLyric(p.cleaned)) score += 5;
    if (/[\u4E00-\u9FFF]/.test(p.cleaned)) score += 4;
    if (/^[0-9]/.test(p.cleaned)) score -= 2;
    if (/�/.test(p.cleaned)) score -= 3;
  }
  return score;
}
...
function isSystemNoise(s) {
  return /has_unlimited_entitlement|logged_in|client\.version|client\.name|lyrics_collection|IOS_MUSIC|entity|collection|button|controller|panel/i.test(s);
}

function isLikelyLyric(s) {
  if (!s) return false;
  if (s.length < 4 || s.length > 80) return false;
  if (isSystemNoise(s)) return false;
  if (/歌詞?b|lyrics?b|Ӎ|˳ɔ|q&Ғ|IOS_MUSIC|logged_in|client\.version|client\.name/i.test(s)) return false;
...
function normalizeTranslation(s) {
  return normalizeLine(String(s || '').replace(/\s*\([^)]*Google[^)]*\)\s*/ig, '').replace(/\s+/g, ' '));
}

function buildBilingualLine(src, tr) {
  const cleanSrc = normalizeLine(src);
  const cleanTr = normalizeTranslation(tr);
  if (!cleanTr) return cleanSrc;
  const short = `${cleanSrc}｜${cleanTr}`;
  if (utf8Bytes(short).length <= utf8Bytes(cleanSrc).length) return short;
  return cleanTr;
}
...
async function translateLine(text, target) {
  try {
    ...
    const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=${encodeURIComponent(target||'zh-CN')}&dt=t&q=${q}`;
    const resp = await $task.fetch({ url, method: 'GET', headers: { 'Accept-Language': 'zh-CN,zh;q=0.9,en;q=0.8' } });
    if (!resp || !resp.body) return '';
    const data = JSON.parse(resp.body);
    if (!Array.isArray(data) || !Array.isArray(data[0])) return '';
    const out = data[0].map(x => Array.isArray(x) ? (x[0] || '') : '').join('').trim();
    return out;
  } catch (e) {
    $.log('translateLine error: ' + String(e));
    return '';
  }
}

function hardSimplifyCommon(str) {
  let x = String(str || '');
  for (const [from, to] of COMMON_T2S_PHRASES) x = x.split(from).join(to);
  return x;
}

function normalizeLine(s) {
  let x = String(s || '').replace(/\s{2,}/g, ' ').trim();
  x = hardSimplifyCommon(x);
  x = x.replace(/怎幺/g, '怎么').replace(/前一秒鍾/g, '前一秒钟').replace(/一秒鍾/g, '一秒钟').replace(/裏/g, '里').replace(/囉嗦/g, '啰嗦').replace(/放鬆/g, '放松').replace(/夢/g, '梦').replace(/隱秘/g, '隐秘');
  return x;
}
function toSimplified(str) { let out=''; for (const ch of String(str)) out += (T2S_MAP[ch] || ch); return hardSimplifyCommon(out); }
